...
X_tr = np.loadtxt('Train/X_train.txt',
                  delimiter=' ').astype(np.float32)
Y_tr = np.loadtxt('Train/y_train.txt').astype(np.float32)
Y_tr_svm = Y_tr
...
X_test = np.loadtxt('Test/X_test.txt',
                    delimiter=' ').astype(np.float32)
Y_test = np.loadtxt('Test/y_test.txt').astype(np.float32)
Y_test_svm = Y_test
...
X = tf.Variable(tf.float32, [None, 561])
Y = tf.Variable(tf.float32, [None, 12])
nhidden = 1024
W1 = tf.Variable(
    0.001*np.random.randn(561, nhidden).astype(np.float32), name='weights')
b1 = tf.Variable(
    0.001*np.random.randn(nhidden).astype(np.float32), name='bias')

W2 = tf.Variable(0.001*np.random.randn(nhidden,
                 12).astype(np.float32), name='weights')
...
cost = tf.reduce_mean(-tf.reduce_sum(Y *
                      tf.log(tf.nn.softmax(pred)+1e-10), reduction_indices=1))
    ...
    for epoch in xrange(training_epochs):
        ...
        for i in xrange(num_batches):
            ...
            _, c = sess.run([optimizer, cost], feed_dict={
                            X: batch_x, Y: batch_y, dropout: 0.5, learning_rate: lr})
        ...
        print('Epoch: %d/%d\t cost: %.8f\t val_acc: %.4f' %
              (epoch+1, training_epochs, curr_loss, val_acc[epoch]))

    print("Training Accuracy: ", accuracy.eval(
        {X: X_train, Y: Y_train, dropout: 1}))
